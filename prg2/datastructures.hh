...
public:
    ...
    // Short rationale for estimate: iterate through affiliations and then nest iteration through
    // vector publications.
    std::vector<Connection> get_connected_affiliations(AffiliationID id);

    // Estimate of performance: O(n^3)
    // Short rationale for estimate: iterate through affiliations and then nest iteration through
    // affiliations and then nest iteration through vector publications.
...
private:
    struct Affiliation {
        ...
        std::unordered_map<Affiliation*, std::pair<Weight, Distance>> to_neighbours;
    };
    struct Publication {
        PublicationID id;
        Name name;
    ...
    void dfs(AffiliationID current, AffiliationID target,
             std::unordered_set<AffiliationID>& visited, Path& result);
};
